...
app.use(function(req, res, next) {
  res.header("Access-Control-Allow-Origin", "https://thunderous-haupia-d8250a.netlify.app/"); // Replace "*" with your allowed domains
...
router1.get("/api/game-categories", async function (req, res, next) {
  try {
    res.json(await gameMethods.getGameCategories());
    console.log('mysql db queries are being used accessing oahi')
  } catch (err) {
...
router2.get("/api/category-clues/:catid", async function (req, res, next) {
  try {
    ...
    console.log(catid);
    res.json(await gameMethods.getCategoryClues(catid));
  } catch (err) {
...
router3.get("/api/games", async function (req, res, next) {
  try {
    res.json(await gameMethods.getGames());
  } catch (err) {
...
router4.get("/api/category-clue/:clue_id", async function (req, res, next) {
  try {
    ...
    console.log(id);
    res.json(await gameMethods.getClue(id));
  } catch (err) {
...
router5.patch(
  ...
  async function (req, res, next) {
    try {
      ...
      res.json(await gameMethods.updateClue(id, answeredCorrect));
    } catch (err) {
...
router6.patch("/api/category-clue/newgame", async function (req, res, next) {
  try {
    res.json(await gameMethods.resetClues());
  } catch (err) {
...
router7.patch("/api/game/:gameid&:score", async function (req, res, next) {
  try {
    ...
    res.json(await gameMethods.setScore(gameid, score));
  } catch (err) {
...
router3.get("/api/allclues", async function (req, res, next) {
  try {
    res.json(await gameMethods.getAllClues());
  } catch (err) {
